...
namespace OpenBYOND.VM
{
    ...
    {
        Regex REGEX_TABS = new Regex(@"^(?P<tabs>[\t\s]*)");
        Regex REGEX_ATOMDEF = new Regex(@"^(?P<tabs>\t*)(?P<atom>[a-zA-Z0-9_/]+)\\{?\\s*$");
        Regex REGEX_ABSOLUTE_PROCDEF = new Regex(@"^(?P<tabs>\t*)(?P<atom>[a-zA-Z0-9_/]+)/(?P<proc>[a-zA-Z0-9_]+)\((?P<args>.*)\)\\{?\s*$");
        Regex REGEX_RELATIVE_PROCDEF = new Regex(@"^(?P<tabs>\t*)(?P<proc>[a-zA-Z0-9_]+)\((?P<args>.*)\)\\{?\\s*$");
...
private  string current_filename;
    ...
    private void consumeVariable(string line, string filename, int ln, out string name, out BYONDValue prop) {
        name="";
        prop=null;
        ...
        if(line.Contains("[") && !line.Contains("list(")) {
                string[] arrparts = line.Split(new char[]{'['}, 1);
        ...
        if(line.Contains("=")) {
            var chunks = line.Split(new char[]{'='}, 1);
            decl=chunks[0].Trim(); 
            value = chunks[1].Trim();
        }else {
            decl = line.Trim();
        ...
        if( typepath != "/") {
            if (value == "null") {
                ...
                prop = (BYONDValue)(new Atom(typepath, filename, ln));//{declarative=declaration, special=special,size=size};
            }
            ...
            prop = new BYONDNumber(float.Parse(value), filename, ln){declarative=declaration, special=special,size=size};
        } else if(value == "null") {
                prop = new BYONDNull(filename, ln){type = new BYONDType(typepath), declarative=declaration, special=special,size=size};
        }
